...
if (isset($_GET['search'])) {
  ...
  $recordCountSql = "SELECT count(*) as Count FROM stock WHERE flavour LIKE '%$searchval%'";
  ...
  $select = "SELECT * FROM stock WHERE flavour LIKE '%$searchval%' LIMIT " . (($pageNo - 1) * $range) . ",$range";
  $query = $connect->query($select);
  $result = $query->fetchAll(PDO::FETCH_ASSOC);
} else {
  $recordCountSql = "SELECT count(*) as Count FROM stock";
  $result = $connect->query($recordCountSql);
  $countRecord = $result->fetchAll(PDO::FETCH_ASSOC)[0]["Count"];
  $pageCount = $countRecord % $range == 0 ? ($countRecord / $range) : intval($countRecord / $range) + 1;
  if (!isset($_GET["page"])) {
  ...
  $select = "SELECT * FROM stock LIMIT " . (($pageNo - 1) * $range) . ",$range";
  $query = $connect->query($select);
  $result = $query->fetchAll(PDO::FETCH_ASSOC);
}
?>
      ...
      <nav class="navbar navbar-expand-lg navbar-transparent  bg-primary  navbar-absolute">
        <div class="container-fluid">
          <div class="navbar-wrapper">
            ...
            <a class="navbar-brand" href="../examples/stock.php">Stock</a>
          </div>
                    ...
                    <ul class="pagination">
                      ...
                      for ($i = 1; $i <= $pageCount; $i++) {
                        if (!isset($_GET["page"]))
                          $_GET["page"] = 1;
                      ?>
                        <li class="page-item <?= $_GET["page"] == $i ? "active" : "" ?>"><a class="page-link" href="stock.php?page=<?= $i ?>&range=<?= $range ?>"><?= $i ?></a></li>
                      <?php
                      }
                      ?>
                    </ul>
